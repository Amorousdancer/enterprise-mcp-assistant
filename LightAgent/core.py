...
# 全局事件循环引用，用于保持 MCP 连接存活
_shared_event_loop: Optional[asyncio.AbstractEventLoop] = None

def set_shared_event_loop(loop: asyncio.AbstractEventLoop):
    """设置共享事件循环（由 app.py 在 MCP 初始化时调用）"""
...
def run_async_safely(coro: Coroutine[Any, Any, T]) -> T:
    ...
    try:
        ...
        # 如果存在共享事件循环（MCP 已初始化且在后台线程运行），使用它
        if _shared_event_loop and not _shared_event_loop.is_closed() and _shared_event_loop.is_running():
            future = asyncio.run_coroutine_threadsafe(coro, _shared_event_loop)
...
class LightAgent:
    ...
    async def setup_mcp(
        ...
        if self.mcp_setting and not self.mcp_client:
            ...
            await self.mcp_client.initialize()          # 建立持久连接
            await self.mcp_client.register_mcp_tool()   # 注册工具到 Registry
    ...
    def _run_non_stream_logic(self, response, max_retry) -> Union[str, None]:
        ...
        for retry_idx in range(max_retry):
            if response.choices[0].message.tool_calls:
                ...
                # 将工具调用和响应添加到消息列表中（转为 dict 避免 ChatCompletionMessage 无 .get()）
